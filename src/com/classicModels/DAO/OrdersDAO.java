...
public class OrdersDAO extends AbstractDAO {
	...
	public String generateWHEREquery(AbstractDTO ob) {
		...
		if (bean.getCustomerNumber() != 0) {
			...
			sb.append("customerNumber LIKE '").append(bean.getCustomerNumber()).append("'");
		}
...
// Methode renvoyant le dernier enregistrement de la table order 
	...
	public String generateDeletequery(AbstractDTO ab) {
		OrdersDTO bean = (OrdersDTO) ab;
		StringBuilder sb = new StringBuilder();

		sb.append("WHERE orderNumber='").append(bean.getOrderNumber()).append("';");
