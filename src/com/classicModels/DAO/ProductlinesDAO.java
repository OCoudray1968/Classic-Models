...
public class ProductlinesDAO extends AbstractDAO {
	...
	public String generateWHEREquery(AbstractDTO plb) {
		...
		if (bean.getProductLine() != "") {
			isWhereClause = true;
			sb.append(" WHERE productLine LIKE '").append(bean.getProductLine()).append("'");
		}
		...
		if (bean.getTextDescription() != null && !bean.getTextDescription().isEmpty()) {
			...
			sb.append("textDescription LIKE '").append(bean.getTextDescription()).append("'");
		}
		...
		if (bean.getHtmlDescription() != null && !bean.getHtmlDescription().isEmpty()) {
			...
			sb.append("htmlDescription LIKE '").append(bean.getHtmlDescription()).append("'");
		}
		...
		if (bean.getImage() != null && !bean.getImage().isEmpty()) {
			...
			sb.append("image LIKE '").append(bean.getImage()).append("'");
		}
		System.out.println("sql where productline :" + sb);
	...
	public String generateUPDATEquery(AbstractDTO ab) {
		...
		// Le nom du contact
		sb.append("textDescription = '").append(bean.getTextDescription()).append("', ");

		// Le vendeur
		sb.append("htmlDescription = '").append(bean.getHtmlDescription()).append("', ");

		// description du produit
		sb.append("image = '").append(bean.getImage()).append("' ");

		// Clause WHERE
		sb.append("WHERE productLine = '").append(bean.getProductLine()).append("' ;");
