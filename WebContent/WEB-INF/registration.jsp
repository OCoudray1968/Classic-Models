...
<div id="corps">
	<c:choose>
		...
		<c:when test="${ empty sessionScope.registrations }">
			<p class="erreur">Aucune demande d'inscription enregistré.</p>
		</c:when>
		<%-- Sinon, affichage du tableau. --%>
		<c:otherwise>
		<form method="post" action="<c:url value="/registrationServlet" />">
			Numéro Demande :<Select name="NumDemande" id="numDemande">
			
							<%-- Parcours de la Map des clients en session, et utilisation de l'objet varStatus. --%>
				<c:forEach items="${ sessionScope.registrations }" var="registrations"
					varStatus="boucle">
			
				 	<%-- Affichage des listes deroulantes demandes --%>
						<option value="${registrations.customerNumber }">${registrations.customerNumber}</option>
